...
// 변수 선언
//시리얼 통신에서 마스터는 보내기만 하기 때문에 TX에는 안쓰는 핀 번호만 적고 기존 사용하던 2번 핀으로 
//IR리시버 핀을 지정할 수 있다.
#define TX 4
#define RX 3
...
void sendData(){
  if(interruptCount==0){
    sendCount = 1;                                                  // sendCount==1 이면 data를 보낸다.
    interruptCount++;                              // interruptCount가 1이 되서 interrupt가 발생해도 수행되는 기능이 없음.
  }
...
void setup() {
  ...
  // irReceiver가 출력할 때 4번의 위상 변화가 발생해 한 번 수신할 때 interrupt가 4번 발생한다.
  // 한 번 수신할 때 interrupt가 한 번만 발생할 수 있도록 interruptCount 로 interrupt 제한.
}
/*
  <Button>     <Mode>
1. Btn CH-   : manual
